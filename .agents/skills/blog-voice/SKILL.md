...
- Put each image, code block, or example where it's discussed, so the reader meets it in context. Introduce a block, then show it. Don't strand a screenshot at the end of a section it illustrated at the top, or drop an artifact before the sentence that sets it up.
